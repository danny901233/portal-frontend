...
export default function PronunciationsTab({ config, save, isSaving }: Props) {
  ...
  return (
    <TabShell
      ...
      description="Tell the agent how to say tricky names, place names, or product terms. Spell it like you'd sound it out."
      onSave={handleSave}
      isSaving={isSaving}
    >
      <div>
        <div className="mb-2 flex items-center justify-between">
          <span className="text-sm font-medium text-slate-700">Pronunciations</span>
        ...
        {items.length === 0 ? (
          <div className="rounded-lg border border-dashed border-slate-300 bg-slate-50 px-4 py-6 text-center text-xs text-slate-500">
            No pronunciations yet. Click &ldquo;Add pronunciation&rdquo; to add one.
          </div>
        ) : (
            ...
            {items.map((item, idx) => (
              <li key={idx} className="grid grid-cols-12 gap-2 rounded-lg border border-slate-200 bg-white p-3">
                <div className="col-span-5">
                  <label className="mb-1 block text-[10px] font-medium uppercase tracking-wide text-slate-500">Written</label>
                  <input
                    type="text"
                    value={item.written}
                    onChange={(e) => updateItem(idx, { written: e.target.value })}
                    placeholder="e.g. Cholmondeley"
                ...
                <div className="col-span-6">
                  <label className="mb-1 block text-[10px] font-medium uppercase tracking-wide text-slate-500">Sound it out</label>
                  <input
                    type="text"
                    value={item.spoken}
