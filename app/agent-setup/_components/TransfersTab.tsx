...
export default function TransfersTab({ config, save, isSaving }: Props) {
  ...
  return (
    <TabShell
      ...
      description="Where to send calls the agent can't handle (complex complaints, warranty claims, etc.). Leave blank to keep all calls AI-handled."
      onSave={handleSave}
      isSaving={isSaving}
    >
      <div>
        <label className="mb-1 block text-sm font-medium text-slate-700">Fallback transfer number</label>
        <input
          type="tel"
        ...
        <p className="mt-1 text-xs text-slate-500">
          Use full international format (e.g. +447123456789).
        </p>
      </div>
      ...
      <label className="flex items-start gap-3 rounded-lg border border-slate-200 bg-slate-50 p-3">
        ...
        />
        <div>
          <p className="text-sm font-medium text-slate-900">Allow the agent to offer a transfer</p>
