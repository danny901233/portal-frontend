...
export type Scenario = 'booking' | 'parts';

// Shared voice-demo UI. Rendered by /demo (scenario="booking") and /partsdemo (scenario="parts").
export default function VoiceDemo({ scenario = 'booking' }: { scenario?: Scenario }) {
  ...
  }, [cleanup, voice, scenario, expressiveOn, ttsKey, agentVariant]);
      ...
      <div className="w-full max-w-lg">
        ...
        <div className="rounded-3xl bg-white/10 p-7 text-center shadow-2xl ring-1 ring-white/15 backdrop-blur-sm sm:p-9">
          ...
          <p className="mx-auto mt-2 max-w-sm text-[15px] leading-relaxed text-brand-100/90">
            {scenario === 'parts'
              ? (live
                  ? 'Order parts like you would from your usual motor factor. Give a reg and the part — they’ll pull the fitment, quote a few options, and “book” it onto the van. It’s a demo, so nothing’s really ordered.'
                  : 'Hear ReceptionMate on a trade parts counter. Pretend you’re a garage ringing to order a part — give a reg, get options and a price, and it’ll take the order. Just a demo.')
              : (live
