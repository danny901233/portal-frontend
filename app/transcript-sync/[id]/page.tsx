...
import { ToolCallEntry } from '../../calls/[id]/components/ToolCallEntry';
import { LogEntry } from '../../calls/[id]/components/LogEntry';
...
export default function CallDetailPage() {
  ...
  // Ref to the audio element so transcript clicks can seek playback.
  ...
  // Normalise a transcript timestamp to seconds-from-audio-start.
  // Some agents record seconds-from-call-start (small numbers, use directly).
  // Others record Unix epoch seconds (huge numbers, subtract call start).
  const callStartMs = new Date(call.createdAt).getTime();
  const normTs = (ts: unknown): number => {
    const n = typeof ts === 'number' ? ts : Number(ts);
    if (!Number.isFinite(n)) return 0;
    if (n < 100000) return n;
    return Math.max(0, n - callStartMs / 1000);
  };
  // Timestamps in the transcript mark END of speech, which equals START of the
  // NEXT entry (assuming no silence gap). So entry N's start-of-speech = entry
  // (N-1)'s stored timestamp. First entry starts at 0 (top of recording).
  const startTimeOf = (index: number): number => {
    if (index <= 0) return 0;
    let prevIdx = index - 1;
    while (prevIdx >= 0) {
      const t = 'type' in transcript[prevIdx] ? (transcript[prevIdx] as { type?: string }).type : 'message';
      if (t === 'message' || t === 'agent_handoff' || !t) break;
      prevIdx--;
    }
    if (prevIdx < 0) return 0;
    return normTs(tsOf(transcript[prevIdx]));
  };
  const jumpToEntry = (index: number) => {
    const audio = audioRef.current;
    if (!audio) return;
    audio.currentTime = startTimeOf(index);
    audio.play().catch(() => { /* autoplay blocked — user needs to press play first */ });
  };
              ...
              >
                {transcript.map((entry: CallRecord['transcript'][number], index) => (
                  <div
                    ...
                    className="cursor-pointer transition-colors hover:bg-slate-50/70 rounded-lg"
                  >
                    <TranscriptEntry
                      entry={entry}
                      allEntries={transcript}
                      offsetSeconds={Math.max(0, Math.round(startTimeOf(index)))}
