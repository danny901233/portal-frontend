...
export default function ChatWidget() {
  ...
  return (
    <>
      ...
      {/* Menu Options */}
      {viewState === 'menu' && (
        <div className="fixed bottom-24 right-6 z-50 w-[340px] animate-in slide-in-from-bottom-4 duration-200 rounded-2xl shadow-2xl overflow-hidden">
          {/* Dark branded header */}
          <div className="px-5 pt-5 pb-5 flex flex-col items-center" style={{ backgroundColor: '#0f172a' }}>
            ...
            <h3 className="text-white font-bold text-base">{config?.name ?? 'Get in Touch'}</h3>
            <p className="text-slate-400 text-xs mt-0.5">We typically reply instantly</p>
          </div>

          {/* White card body */}
          <div className="bg-white px-4 pt-4 pb-2">
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Message us on...</p>
            ...
            <div className="space-y-2">
              ...
              {config.whatsappNumber && (
                <button
                  ...
                  className="w-full flex items-center gap-3 px-4 py-3.5 rounded-xl transition-all bg-white hover:bg-gray-50 active:scale-[0.98] border border-blue-200 hover:border-blue-400"
                >
              ...
              {config.phone && (
                <button
                  ...
                  className="w-full flex items-center gap-3 px-4 py-3.5 rounded-xl transition-all bg-white hover:bg-gray-50 active:scale-[0.98] border border-blue-200 hover:border-blue-400"
                >
                  <div className="w-9 h-9 bg-violet-500 rounded-full flex items-center justify-center flex-shrink-0">
                    <svg className="w-5 h-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          {/* Powered by footer — outside white card */}
          <div className="bg-white px-4 pt-2 pb-4 text-center">
      ...
      <button
        ...
        className="fixed bottom-6 right-6 z-50 h-12 px-5 rounded-full flex items-center gap-2.5 transition-all duration-300 hover:scale-105 active:scale-95 whitespace-nowrap"
        style={{ backgroundColor: '#0f172a', boxShadow: '0 8px 32px rgba(0,0,0,0.4)' }}
      ...
      >
        {viewState === 'closed' ? (
          <>
            ...
            <span className="text-white font-bold text-sm">Chat now!</span>
          </>
        ) : (
          <>
            <svg className="w-4 h-4 text-white flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <span className="text-white font-bold text-sm">Close</span>
          </>
        )}
      </button>
    </>
