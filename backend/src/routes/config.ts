...
const updateSipDispatchRule = async (garageId: string, agentScript: string) => {
...
router.put(
  ...
  async (req: Request, res: Response) => {
    ...
    let resolvedAgentScript: string = data.agentScript || 'receptionmate-agent';
    ...
    if (!canEditAgentType) {
      ...
      resolvedAgentScript = existingConfig?.agentScript || 'receptionmate-agent';
    }
