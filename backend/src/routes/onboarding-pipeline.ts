...
import { setOnboardingStage, ONBOARDING_STAGES } from '../utils/onboardingStage.js';
import {
...
//
// RECOVERED 2026-09-09 from dist/routes/onboarding-pipeline.js. Never committed, so it was lost
// when the box became a clean checkout in August; the routes have been 404 since. Note that a
// GarageHive garage now gets its login automatically at go-live (see garageHiveConnect.ts), so
// the invite below is the manual path for everyone else and the re-send for anyone missed.
// ---------------------------------------------------------------------------
const router = Router();
...
async function resolveCustomerUser(garageId: string) {
  ...
  const manager = users.find((u) => {
    const roles = (u.branchRoles ?? {}) as Record<string, unknown>;
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    if (!user) {
      return res
        .status(400)
        .json({ error: 'No customer user found for this garage (staff accounts are excluded).' });
    }
  ...
  async (_req: Request, res: Response) => {
    const garages = await prisma.garage.findMany({
      ...
      select: {
        ...
        agentConfiguration: {
          select: { agentType: true, agentScript: true, integrationProvider: true },
        },
      },
    });
    const rows = await Promise.all(
      garages.map(async (g) => {
        ...
        const cfg = Array.isArray(g.agentConfiguration)
          ? g.agentConfiguration[0]
          : g.agentConfiguration;
        return {
          garageId: g.id,
          garageName: g.name,
...
router.get(
  '/admin/highlevel/opportunities',
  authenticate,
  requireAdmin,
  async (req: Request, res: Response) => {
    if (!highlevelConfigured())
      return res.json({ configured: false, candidates: [], suggestedId: null });
    ...
    const candidates: OpportunityCandidate[] = await findOpportunityCandidates({ email, phone });
      ...
      if (pending?.ghlOpportunityId) {
        ...
        if (candidates.some((c) => c.id === knownId)) {
          ...
          if (one) {
            ...
            // staff pick.
            console.warn(
              `[PIPELINE] stored HL opportunity ${knownId} for ${email} no longer exists — not suggesting it`,
            );
          }
        }
      }
...
router.patch(
  '/admin/garages/:garageId/highlevel',
  authenticate,
  requireAdmin,
  async (req: Request, res: Response) => {
    const parsed = linkSchema.safeParse(req.body);
    if (!parsed.success)
      return res.status(400).json({ error: 'ghlOpportunityId must be a string or null.' });
    const garage = await prisma.garage.findUnique({
      where: { id: req.params.garageId },
      select: { id: true },
    });
