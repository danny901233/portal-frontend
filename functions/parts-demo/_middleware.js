// Cloudflare Pages Function — gates /parts-demo behind the same shared password as /lkqdemo.
//
// The parts demo exists at two URLs: /lkqdemo/parts-demo.html, which has been gated since it
// was built, and /parts-demo at the site root, which was not — so the whole LKQ demo was
// publicly reachable by anyone who guessed the path. Same password and same cookie token as
// the lkqdemo gate, deliberately: signing in once covers both, and there is no second secret
// to keep in step.
//
// (original header) Gates behind a single shared password
// with a branded login page (no username). The password and session token live
// only in this server-side function, never sent to the client. Mirrors /fixmycar.

const PASSWORD = "Nomoremissedsales";
const COOKIE = "lkq_auth";
const TOKEN = "lkq-9f4c1a7e2b8d305641af";
